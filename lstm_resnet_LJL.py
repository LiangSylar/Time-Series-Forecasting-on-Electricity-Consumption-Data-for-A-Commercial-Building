...
folderName = 'test_h108_l2/'
# p = 'corrected_753.csv'
p = 'raw_data/CL_imputed.csv'
...
def split_sequences(sequences, n_steps):
    ...
    for i in range(len(sequences)):
        ...
        seq_x_f = sequences[end_ix, 0:(n_features-1)]
        X.append(seq_x)
        y.append(seq_y)
        x_f.append(seq_x_f)
    return np.array(X), np.array(y), np.array(x_f) ### 假设一个seq长度为T，则x_f记录着T+1时刻的features数值，y记录着T+1时刻的CL数值
...
class MV_LSTM(torch.nn.Module):
    def __init__(self,n_features,seq_length):
        ...
        self.hist_reg = torch.nn.Linear(self.n_hidden, 1) ### prediction  
        self.feat_reg = torch.nn.Linear(n_features-1, 1) ### 利用自变量; 最后一个是因变量，所以-1  
        self.finalReg = torch.nn.Linear(2, 1) ### 对y1 y2进行最后的计算 
    ...
    def forward(self, x, x_f):        
        ...
        # print(x_f.shape) # torch.Size([64, 7])         
        lstm_out, self.hidden = self.l_lstm(x,self.hidden)
        # lstm_out(with batch_first = True) is 
        # (batch_size,seq_len,num_directions * hidden_size)
        # for following linear layer we want to keep batch_size dimension and merge rest       
        # .contiguous() -> solves tensor compatibility error
        # print(lstm_out.shape) # [64, 96, 32]:[batch_size, seq_len, dim_h]
        # print(self.hidden[1].shape) # [2, 64, 32]:[num_layers, batch_size, dim_h]
        h = lstm_out[:, -1, :]
        # print(h.shape) # [64, 32]
        y_1 = self.hist_reg(h) 
        # print(y_1.shape) # [64, 1] 
        y_2 = self.feat_reg(x_f)
        # print(y_2.shape) # [64, 1]
        y_final = self.finalReg(torch.cat([y_1, y_2], dim=1))
        # print(y_final.shape) # [64, 1]

        # x = lstm_out.contiguous().view(batch_size,1,n_timesteps,self.n_hidden) ### 相当于unsqueeze，为了配合resnet输入
        # x = self.resmodel(x) # [64, 1, 96, 64]
        # exit(0)
        return y_final
        
X, Y, X_F = split_sequences(data, n_timesteps)
#X = np.expand_dims(X, axis=1)
print(data.shape) # (52608, 8)
print('X.shape = ', X.shape) # (52512, 96, 8)
print('Y.shape = ',Y.shape) # (52512,)
print('X_f.shape = ', X_F.shape) # (52512, 7)
# exit(0)
print(y_std)
...
X_train = X[:X.shape[0]-672]
X_test = X[X.shape[0]-672:]
X_test[:,:,-1] = 0

y_train = Y[:X.shape[0]-672]
y_test = Y[X.shape[0]-672:]

X_F_train =  X_F[:X.shape[0]-672]
X_F_test = X_F[X.shape[0]-672:] 
...
for t in range(train_episodes):
    ...
    for b in range(0,len(X_test)):
        ...
        inpt = X_test[b:b+1,:,:]
        target = y_test[b:b+1]    
        feature = X_F_test[b:b+1,:]
        
        x_batch = torch.tensor(inpt,dtype=torch.float32).to(device)   
        y_batch = torch.tensor(target,dtype=torch.float32).to(device)
        x_f_batch = torch.tensor(feature,dtype=torch.float32).to(device)
    
        output_test = mv_net(x_batch, x_f_batch)
    ...
    #plt.show()
    plt.savefig(folderName + 'ep{}_bs{}_lr{}_layer{}_hidden{}.jpg'.format(t,batch_size,lr_rate,2,64))
    plt.close()
    if (math.sqrt(np.mean(test_loss))*y_std<best_loss):
        best_loss = math.sqrt(np.mean(test_loss))*y_std
        best_ep = t
    train_rmse = math.sqrt(np.mean(train_loss))*y_std
    test_rmse =  math.sqrt(np.mean(test_loss))*y_std
    print('step : ' , t , 'train_rmse : ' , train_rmse, 'test_rmse : ' , test_rmse, 'best_loss : ' , best_loss, 'test_ep : ' , best_ep)
    ...
    plt.title("RMSE")
    plt.savefig(folderName + 'prediction RMSE.png')
    plt.show()