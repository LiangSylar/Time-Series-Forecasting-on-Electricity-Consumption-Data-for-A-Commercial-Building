...
import csv
import math
p1 = 'train_ground_truth_12_29.csv'
p2 = 'train_missing_value_12_29.csv'
p3 = 'test_ground_truth_12_29.csv'
p4 = 'test_missing_value_12_29.csv'
with open(p1,encoding = 'utf-8') as f:
    data1 = np.loadtxt(f,str,delimiter = ",",skiprows = 1,usecols = (1,2,3,4,7))
...
with open(p3,encoding = 'utf-8') as f:
    data3 = np.loadtxt(f,str,delimiter = ",",skiprows = 1,usecols = (1,2,3,4,7))
data3 = data3.astype('float')

for i in range(data3.shape[1]-1):
    data3[:,i] = (data3[:,i]-np.mean(data3[:,i]))/np.std(data3[:,i])
...
for i in range(data3.shape[0]):
  for j in range(data3.shape[1]):
    if data[i,j] != "":
      data4[i,j] = float(data[i,j])

for i in range(data4.shape[1]-1):
    data4[:,i] = (data4[:,i]-np.mean(data4[:,i]))/np.std(data4[:,i])
...
def main(argv):
    ...
    # Load data #
    #############
    x_train_miss, y = split_sequences(data2, time_length) 
    x_train_full, y_train = split_sequences(data1, time_length)
    # data = np.load(FLAGS.data_dir)
    m_train_miss = x_train_full!=x_train_miss
    # x_train_full = data['x_train_full']
    # x_train_miss = data['x_train_miss']
    # m_train_miss = data['m_train_miss']
    # if FLAGS.data_type in ['hmnist', 'physionet']:
    #     y_train = data['y_train']
    x_test_full, y_val = split_sequences(data3, time_length)
    x_test_miss, y = split_sequences(data4, time_length) 
    m_val_miss = x_test_full!=x_test_miss
